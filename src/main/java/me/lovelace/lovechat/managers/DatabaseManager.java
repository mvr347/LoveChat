...
public class DatabaseManager {
    ...
    public CompletableFuture<Integer> getMessageCount(UUID uuid) { return CompletableFuture.supplyAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("SELECT message_count FROM player_stats WHERE uuid = ?")) { pstmt.setString(1, uuid.toString()); try (ResultSet rs = pstmt.executeQuery()) { if (rs.next()) return rs.getInt("message_count"); } } catch (SQLException e) { logSqlWarning("reading message count", e); } return 0; }, dbExecutor); }
    public void incrementMessageCount(UUID uuid) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("INSERT INTO player_stats (uuid, message_count) VALUES (?, 1) ON CONFLICT(uuid) DO UPDATE SET message_count = message_count + 1")) { pstmt.setString(1, uuid.toString()); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("incrementing message count", e); } }, dbExecutor); }
    public void clearAllMessagesSync() { try (Statement stmt = connection.createStatement()) { stmt.executeUpdate("DELETE FROM messages"); } catch (SQLException e) { logSqlWarning("clearing messages", e); } }
    public void deleteMessage(int id) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("DELETE FROM messages WHERE id = ?")) { pstmt.setInt(1, id); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("deleting message " + id, e); } }, dbExecutor); }
    public void updateMessage(int id, String text) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("UPDATE messages SET content = ? WHERE id = ?")) { pstmt.setString(1, text); pstmt.setInt(2, id); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("updating message " + id, e); } }, dbExecutor); }
    public void cleanOldMessages(long age) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("DELETE FROM messages WHERE timestamp < ?")) { pstmt.setLong(1, System.currentTimeMillis() - age); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("cleaning old messages", e); } }, dbExecutor); }
    ...
    public void removeIgnore(UUID who, UUID target) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("DELETE FROM ignores WHERE uuid_from = ? AND uuid_to = ?")) { pstmt.setString(1, who.toString()); pstmt.setString(2, target.toString()); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("removing ignore " + who + " -> " + target, e); } }, dbExecutor); }
    public void clearIgnores(UUID who) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("DELETE FROM ignores WHERE uuid_from = ?")) { pstmt.setString(1, who.toString()); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("clearing ignores for " + who, e); } }, dbExecutor); }
    ...
    public void clearDefaultChannel(UUID uuid) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("DELETE FROM player_settings WHERE uuid = ?")) { pstmt.setString(1, uuid.toString()); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("clearing default channel for " + uuid, e); } }, dbExecutor); }
    ...
    public void clearTagsDisabled(UUID uuid) { CompletableFuture.runAsync(() -> { try (PreparedStatement pstmt = connection.prepareStatement("DELETE FROM player_tags WHERE uuid = ?")) { pstmt.setString(1, uuid.toString()); pstmt.executeUpdate(); } catch (SQLException e) { logSqlWarning("clearing tags-disabled for " + uuid, e); } }, dbExecutor); }
    public void close() { try { if (connection != null && !connection.isClosed()) connection.close(); dbExecutor.shutdown(); } catch (SQLException e) { logSqlWarning("closing database connection", e); } }
